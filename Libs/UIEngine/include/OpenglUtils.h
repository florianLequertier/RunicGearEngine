...
public:
	...
	void setDatas(const std::vector<Vertex>& vertices, const std::vector<GLuint>& indices)
	{
		m_vertices = vertices;
		m_indices = indices;
...
public:
	...
	void create(FT_Face& face)
	{
		...
		m_atlas.m_fontTexture = Texture::create_FontAtlasTex(texWidth, texHeight, textureDatas);
